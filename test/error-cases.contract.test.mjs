...
test('error cases: mint input validation, gate restrictions, and supply cap', async () => {
  ...
  try {
    ...
    const gateArtifact = getArtifact(artifacts, 'ProofGateTest13');
    const mintArtifact = getArtifact(artifacts, 'PhilTestMint');

    const gate = await deployContract(deployer, gateArtifact, [
      PROGRAM_HASH,
      DROP_ID,
      expectedMint,
    ]);
    ...
    const validProof = await generateLocalMintProof({
    ...
    const expiredProof = await generateLocalMintProof({
      signer: backendSigner,
      programHash: PROGRAM_HASH,
      dropId: DROP_ID,
      chainId: CHAIN_ID,
    ...
    const proof14 = await generateLocalMintProof({
      signer: backendSigner,
      programHash: PROGRAM_HASH,
      dropId: DROP_ID,
      chainId: CHAIN_ID,
