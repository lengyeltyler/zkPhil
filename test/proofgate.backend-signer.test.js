...
async function buildFixture() {
  ...
  const backendProofSigner = ethers.HDNodeWallet.fromPhrase(
    'test test test test test test test test test test test junk',
    undefined,
    "m/44'/60'/0'/0/1"
  ).connect(provider);
  const backendSigner = await provider.getSigner(1);
  const recipientSigner = await provider.getSigner(2);
  ...
  await (await gate.setAllowlistSigner(await backendSigner.getAddress())).wait();
...
describe('ProofGateTest13 backend-signer mint verification', function () {
  this.timeout(120000);

  it('accepts a valid backend-signed mint proof and mints', async () => {
    const fixture = await buildFixture();
    const recipient = await fixture.recipientSigner.getAddress();

    const proof = await generateLocalMintProof({
      signer: fixture.backendProofSigner,
  ...
  it('rejects a mint proof signed by the wrong signer', async () => {
    const fixture = await buildFixture();
    const recipient = await fixture.recipientSigner.getAddress();

    const proof = await generateLocalMintProof({
      signer: fixture.wrongSigner,
      programHash: PROGRAM_HASH,
      dropId: DROP_ID,
      chainId: CHAIN_ID,
      proofGateAddress: fixture.gateAddress,
    ...
    await assert.rejects(
      ...
      /InvalidSignature|execution reverted/
    );
  });

  it('rejects expired backend-signed mint proofs', async () => {
    const fixture = await buildFixture();
    const recipient = await fixture.recipientSigner.getAddress();
  ...
  it('rejects replaying the same backend-signed mint proof', async () => {
    const fixture = await buildFixture();
    const recipient = await fixture.recipientSigner.getAddress();
