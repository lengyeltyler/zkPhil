...
export class PhilDatabase {
  ...
  private init() {
    ...
    this.ensureEligibilityEntitlementsTable();
  }
  ...
  private createIssuedMintProofIndexes() {
    this.db.exec(`
      ...
      CREATE INDEX IF NOT EXISTS idx_issued_mint_proofs_signature ON issued_mint_proofs(signature);
    `);
  }

  private ensureEligibilityEntitlementsTable() {
    const legacyTable = this.db
      .prepare(
        `SELECT name FROM sqlite_master WHERE type = 'table' AND name = 'allowlist_entitlements'`
      )
      .get() as { name?: string } | undefined;

    if (legacyTable?.name) {
      this.db.exec(`
        ALTER TABLE allowlist_entitlements RENAME TO eligibility_entitlements;
      `);
    }

    this.db.exec(`
      CREATE TABLE IF NOT EXISTS eligibility_entitlements (
        context_id TEXT NOT NULL,
        address TEXT NOT NULL,
        remaining INTEGER NOT NULL CHECK (remaining >= 0),
        PRIMARY KEY (context_id, address)
      );

      CREATE INDEX IF NOT EXISTS idx_eligibility_entitlements_address
        ON eligibility_entitlements(address);
    `);
  }
  ...
  consumeEligibilityEntitlementAndCreateIssuedMintProof(
    contextId: string,
    address: string,
    ...
    const tx = this.db.transaction(() => {
      ...
      const updateStmt = this.db.prepare(`
        UPDATE eligibility_entitlements
        SET remaining = remaining - 1
        WHERE context_id = ? AND address = ? AND remaining > 0
      `);
      const result = updateStmt.run(contextId, normalizedAddress);
      if (result.changes !== 1) {
        return null;
      }
      ...
      const row = this.db.prepare(`
        SELECT remaining
        FROM eligibility_entitlements
        WHERE context_id = ? AND address = ?
        LIMIT 1
      `).get(contextId, normalizedAddress) as { remaining: number } | undefined;
  ...
  hasActiveIssuedMintProofSignature(signature: string, now: number): boolean {
    const tx = this.db.transaction(() => {
      this.deleteExpiredIssuedMintProofs(now);
      const stmt = this.db.prepare(`
        SELECT 1
        FROM issued_mint_proofs
        WHERE signature = ?
        LIMIT 1
      `);
      const row = stmt.get(signature.toLowerCase()) as { 1: number } | undefined;
      return Boolean(row);
    });
  ...
  ensureEligibilityEntitlement(contextId: string, address: string, initialRemaining: number): void {
    if (!Number.isInteger(initialRemaining) || initialRemaining < 0) {
      throw new Error(`Invalid eligibility entitlement: ${initialRemaining}`);
    }

    const stmt = this.db.prepare(`
      INSERT INTO eligibility_entitlements (context_id, address, remaining)
      VALUES (?, ?, ?)
      ON CONFLICT(context_id, address) DO NOTHING
    `);
    stmt.run(contextId, address.toLowerCase(), initialRemaining);
  }

  getEligibilityRemaining(contextId: string, address: string): number {
    const stmt = this.db.prepare(`
      SELECT remaining
      FROM eligibility_entitlements
      WHERE context_id = ? AND address = ?
      LIMIT 1
    `);
    const row = stmt.get(contextId, address.toLowerCase()) as { remaining: number } | undefined;
    return row?.remaining ?? 0;
  }

  consumeEligibilityEntitlement(contextId: string, address: string): number | null {
    const tx = this.db.transaction(() => {
      const normalizedAddress = address.toLowerCase();
      const updateStmt = this.db.prepare(`
        UPDATE eligibility_entitlements
