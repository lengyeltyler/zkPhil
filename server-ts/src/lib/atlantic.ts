 ...
 *             eligibility_root: felt252, context_id: felt252, recipient: felt252)
 */
function serializeInputsJson(inputs: ProofRequest['inputs']): string {
  const payload = {
    ...
    eligibility_root: inputs.eligibilityRoot,
    context_id: inputs.contextId,
...
function normalizeInputs(inputs: ProofRequest['inputs'], format: InputFormat): ProofRequest['inputs'] {
  ...
  return {
    ...
    eligibilityRoot: hexToDec(inputs.eligibilityRoot),
    contextId: hexToDec(inputs.contextId),
...
function serializeInputsFlat(inputs: ProofRequest['inputs']): string {
  ...
  values.push(inputs.eligibilityRoot);

  // context_id: felt252
  values.push(inputs.contextId);
...
export class AtlanticClient {
  ...
  async submitProof(request: ProofRequest): Promise<ProofResponse> {
    ...
    const inputPreview = {
      ...
      eligibilityRoot: redactHex(request.inputs.eligibilityRoot),
      contextId: redactHex(request.inputs.contextId),
...
export class MockAtlanticClient {
  ...
  async submitProof(request: ProofRequest): Promise<ProofResponse> {
    ...
    this.jobs.set(jobId, {
      ...
      outputs: [
        request.inputs.eligibilityRoot,
        request.inputs.contextId,
