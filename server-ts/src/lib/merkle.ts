...
/**
 * Compute nullifier from secret, contextId, and recipient
 * nullifier = pedersen(DOMAIN_NULL, pedersen(secret, pedersen(contextId, recipient)))
 */
export function computeNullifier(secret: bigint, contextId: bigint, recipient: bigint): bigint {
  const inner = pedersen(contextId, recipient);
