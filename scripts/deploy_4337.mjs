 ...
 *
 * Requires existing deployment of PhilTestMint (from deploy_stark.mjs).
 *
 ...
 *   PROOF_GATE               - Deployed ProofGateTest13 address (or reads from deployments/)
...
export function readDeploy4337Config(env = process.env) {
  ...
  return {
    ...
    privateKey,
    paymasterSignerKey,
    philTestMint: String(env.PHIL_TEST_MINT || '').trim(),
...
export async function runDeploy4337(config = readDeploy4337Config(process.env)) {
  ...
  const philTestMintAddr = config.philTestMint || existingStark.PhilTestMint || '';
  if (!philTestMintAddr) {
    throw new Error(
      'PhilTestMint address not found. Set PHIL_TEST_MINT env var or deploy via deploy_stark.mjs first.'
    );
  }

  const proofGateAddr = config.proofGate || existingStark.ProofGateTest13 || existingStark.ProofGate || '';
  if (!proofGateAddr) {
    throw new Error(
  ...
  if (config.reuse4337) {
    ...
    if (
      existing.data.PhilTestMint &&
      philTestMintAddr &&
      existing.data.PhilTestMint.toLowerCase() !== philTestMintAddr.toLowerCase()
    ) {
      throw new Error(
        `Existing 4337 deployment is pinned to a different PhilTestMint address.\n` +
        `  Existing PhilTestMint: ${existing.data.PhilTestMint}\n` +
        `  Current  PhilTestMint: ${philTestMintAddr}\n` +
  ...
  console.log(`PhilTestMint:      ${philTestMintAddr}`);
  console.log(`ProofGateTest13:   ${proofGateAddr}`);
  ...
  const deployments = {
    ...
    PhilTestMint: philTestMintAddr,
    ProofGateTest13: proofGateAddr,
  };
