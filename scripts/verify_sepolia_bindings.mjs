...
export function readVerifySepoliaBindingsConfig(env = process.env, rootDir = ROOT_DIR) {
  ...
  return {
    ...
    philTestMintAddress: requireAddress(
      'PHIL_TEST_MINT',
      env.PHIL_TEST_MINT || starkDeployment.PhilTestMint || aaDeployment.PhilTestMint
    ),
    ...
    allowlistSignerAddress: resolveSignerAddress(env, {
      label: 'ProofGate.allowlistSigner()',
      addressEnvKey: 'ALLOWLIST_SIGNER',
      privateKeyEnvKey: 'ALLOWLIST_SIGNER_KEY',
      fallbackAddress: starkDeployment.allowlistSigner,
    }),
...
export async function runVerifySepoliaBindings(
  ...
  await withRpcRetry('verify sepolia bindings', async () => {
    ...
    await assertContractCode(provider, 'PhilTestMint', config.philTestMintAddress);
    ...
    const onchainAllowlistSigner = ethers.getAddress(await proofGate.allowlistSigner());
    const onchainFactoryProofGate = ethers.getAddress(await factory.proofGate());
    const onchainFactoryPhilTestMint = ethers.getAddress(await factory.philTestMint());
    ...
    const onchainPaymasterPhilTestMint = ethers.getAddress(await paymaster.philTestMint());
    ...
    assertAddressMatch(
      'ProofGate.allowlistSigner()',
      config.allowlistSignerAddress,
      onchainAllowlistSigner
    );
    assertEnabled(
      'ProofGate.authorizedCaller(PHIL_TEST_MINT)',
      Boolean(await proofGate.authorizedCaller(config.philTestMintAddress))
    );
    assertEnabled(
    ...
    console.log(`  ProofGate.allowlistSigner(): ${onchainAllowlistSigner}`);
    console.log(`  ProofGate.authorizedCaller(PHIL_TEST_MINT): true`);
    ...
    console.log(`  PHIL_ACCOUNT_FACTORY.philTestMint(): ${onchainFactoryPhilTestMint}`);
    ...
    console.log(`  PHIL_PAYMASTER.philTestMint(): ${onchainPaymasterPhilTestMint}`);
